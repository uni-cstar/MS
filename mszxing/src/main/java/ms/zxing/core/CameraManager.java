...
public final class CameraManager {
	...
	/**
	 * A single preview ms.frame will be returned to the handler supplied. The data
	 ...
	 */
	/*
	 * public Point[] convertResultPoints(ResultPoint[] points) { Rect ms.frame =
	 * getFramingRectInPreview(); int count = points.length; Point[] output =
	 * new Point[count]; for (int x = 0; x < count; x++) { output[x] = new
	 * Point(); output[x].x = ms.frame.left + (int) (points[x].getX() + 0.5f);
	 * output[x].y = ms.frame.top + (int) (points[x].getY() + 0.5f); } return
	 * output; }
	 */
