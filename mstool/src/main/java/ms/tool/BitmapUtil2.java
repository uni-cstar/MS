...
public class BitmapUtil2 {
    ...
    public static int getInSampleSize(int outW, int outH, int maxW, int maxHeight) {
        ...
        return var4 > var5 ? (int) ((outW * 1.0) / maxW) : (int) ((outH * 1.0) / maxHeight);
    }
     ...
     * @throws IOException
     */
    public static Date readPictureTakeDate(String path) throws IOException {
        int degree;
        ExifInterface exifInterface = new ExifInterface(path);
        String dateStr = exifInterface.getAttribute(ExifInterface.TAG_DATETIME);
        if(TextUtils.isEmpty(dateStr)){
            return null;
        }else{
            try {
                return DateFormat.getInstance().parse(path);
            }catch (Exception e){
                e.printStackTrace();
                return null;
            }
